...
local function easing(es,o,t,d,from,to)
	if (es < 0) then
		...
		return easing_a(ea,timeCalc(o,t,d),from,to-from,(o.index * d) + t)
	else
		return easing_b(es,from,to,(timeCalc(o,t,d)/((o.index * d) + t)))
	end
end
...
local function Move(obj, time, delay, data)
	if (isValid(data)) then
		if (timeIn(obj, time, delay)) then
			...
			debug_print(easing(eas, obj, time, delay, sx, ex))
...
local function RandomPos(obj, time, delay, data)
	if (isValid(data)) then
		if (timeIn(obj, time, delay)) then
			...
			local dirX = rand(0,1,seed * (obj.index+1),1)
			local dirY = rand(0,1,seed * (obj.index+1),2)
